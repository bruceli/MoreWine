    ...
    if ([self shouldUpdate])
    {
        ...
        if (async)
        {
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
				...
				NSString* pathString = [basePath stringByAppendingPathComponent:@"myfile.png"];
				[binaryImageData writeToFile:pathString atomically:YES];
				NSLog(@"Write file %@",pathString);
				//====
